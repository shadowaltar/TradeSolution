...
public class Execution : IExternalExecutionManagement, ISupportFakeOrder
{
    ...
    public Execution(IExternalConnectivityManagement connectivity, HttpClient httpClient, KeyManager keyManager)
    {
    ...
    private async Task<ExternalQueryState> SendOrder(string url, Order order)
    {
        ...
        var state = ExternalQueryStates.SendOrder(order, responseString, connId, isOk).RecordTimes(rtt, swTotal);
        ...
        static void Parse(JsonNode json, Order order, out Trade[]? fills)
        {
            fills = null;
            ...
            if (fillsObj != null && fillsObj?.Count != 0)
            {
                ...
                for (int i = 0; i < fillsObj!.Count; i++)
                {
                    ...
                    fills[i] = (new Trade
                    {
                        ...
                        FeeAsset = item.GetString("commissionAsset"),
    ...
    public async Task<ExternalQueryState> CancelOrder(Order order)
    {
        ...
        var state = ExternalQueryStates.CancelOrders(order.SecurityCode, responseString, connId, order).RecordTimes(rtt, swTotal);
    ...
    {
        if (!ValidateSecurity(security, ExternalActionType.CancelOrder, out var errorState))
            return errorState;
        ...
        var orders = ParseOrders(responseString, security);
        var connId = ResponseUtils.CheckHeaders(response);
        return ExternalQueryStates.CancelOrders(security.Code, responseString, connId, orders).RecordTimes(rtt, swTotal);
    }
    ...
    {
        if (!ValidateSecurity(security, ExternalActionType.GetTrade, out var errorState))
            return errorState;
        ...
        var connId = ResponseUtils.CheckHeaders(response);
        var trades = Parse(responseString, security);
        var state = trades == null
            ? ExternalQueryStates.InvalidTrade(responseString, connId, $"Failed to get market trades for symbol {security.Code}")
            : ExternalQueryStates.QueryTrades(responseString, connId, trades);
        state.RecordTimes(rtt, swOuter);
        return state!;


        static Trade[]? Parse(string json, Security security)
        {
            var rootArray = JsonNode.Parse(json)?.AsArray();
            if (rootArray == null)
                return null;
            var trades = new Trade[rootArray.Count];
    ...
    {
        if (!ValidateSecurity(security, ExternalActionType.GetOrder, out var errorState))
            return errorState;

        if (orderId <= 0 && externalOrderId <= 0)
        {
            return ExternalQueryStates.InvalidArgument(ExternalActionType.GetOrder, "Must provide either order id or external order id.");
        }
        ...
        ResponseUtils.CheckHeaders(response);
        ExternalQueryState state;
        var order = Parse(responseString, security);
        if (order == null)
            state = ExternalQueryStates.InvalidOrder(responseString, ResponseUtils.GetUniqueConnectionId(response), $"Failed to get order for symbol {security.Code} by orderId {orderId} or externalOrderId {externalOrderId}");
        else
            state = ExternalQueryStates.QueryOrder(order, responseString, ResponseUtils.GetUniqueConnectionId(response));
        state.RecordTimes(rtt, swOuter);
        return state;
    ...
    public async Task<ExternalQueryState> GetOpenOrders(Security? security = null)
    {
        ...
        var connId = ResponseUtils.CheckHeaders(response);
        var orders = ParseOrders(responseString, security);
        var state = ExternalQueryStates.QueryOrders(security?.Code, responseString, connId, orders);
        state.RecordTimes(rtt, swOuter);
        return state!;
    }
    ...
    public async Task<ExternalQueryState> GetOrderSpeedLimit()
    {
        ...
        using var request = _requestBuilder.BuildSigned(HttpMethod.Delete, url);
        ...
        RequestFrequencyInfo?[] Parse(string json)
        {
            var rootObj = JsonNode.Parse(json)?.AsArray();
            if (rootObj == null) return Array.Empty<RequestFrequencyInfo?>();

            var infoArray = new RequestFrequencyInfo?[rootObj.Count];
            for (int i = 0; i < rootObj.Count; i++)
            {
                JsonNode? node = rootObj[i];
                ...
                try
                {
                    info = new RequestFrequencyInfo(
                        rootObj.GetString("rateLimitType"),
                        IntervalTypeConverter.Parse(rootObj.GetString("interval")),
                        rootObj.GetInt("intervalNum"),
                        rootObj.GetInt("count"),
                        rootObj.GetInt("limit")
                    );
                }
                catch (Exception ex)
                {
                                                    ...
                                                    DateTime? end = null)
    {
        ...
        if (!orderId.IsValid() && start == null && end == null)
            return ExternalQueryStates.InvalidArgument(ExternalActionType.GetTrade, "Must provide at least an orderId, start or end time.");

        if (start == null && end != null)
            return ExternalQueryStates.InvalidArgument(ExternalActionType.GetTrade, "If end time is provided, start time must also be provided.");
        ...
        var (response, responseString, rtt) = await _httpClient.TimedSendAsync(request, _log);
        var connId = ResponseUtils.CheckHeaders(response);
        var trades = ParseTrades(responseString, security);
        return ExternalQueryStates.QueryTrades(responseString, connId, trades).RecordTimes(rtt, swOuter);
    }

    private static bool ValidateSecurity(Security? security, ExternalActionType actionType, [NotNullWhen(false)] out ExternalQueryState? errorState)
    {
        errorState = null;
        if (security == null)
        {
    ...
    private static Order[] ParseOrders(string json, Security? security)
    {
        var rootObj = JsonNode.Parse(json)?.AsArray();
        if (rootObj == null)
            return new Order[0];

        var orders = new Order[rootObj.Count];
        for (int i = 0; i < rootObj.Count; i++)
        {
            JsonNode? node = rootObj[i];
            var obj = node?.AsObject();
    ...
    private Trade? ParseTrade(JsonObject? rootObj, int? securityId)
    {
        ...
        try
        {
            return new Trade
            {
                ...
                FeeAsset = rootObj.GetString("commissionAsset"),
    ...
    private Trade[] ParseTrades(string json, Security? security)
    {
        var rootObj = JsonNode.Parse(json)?.AsArray();
        if (rootObj == null) return Array.Empty<Trade>();

        var trades = new Trade[rootObj.Count];
        for (int i = 0; i < rootObj.Count; i++)
        {
            JsonNode? node = rootObj[i];
            var obj = node?.AsObject();
