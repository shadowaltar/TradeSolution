...
public class Execution : IExternalExecutionManagement, ISupportFakeOrder
{
            ...
            if (fillsObj != null && fillsObj.Count != 0)
            {
                ...
                for (int i = 0; i < fillsObj.Count; i++)
                {
                    ...
                    fills[i] = new Trade
                    {
                        ...
                        // TODO externalTradeId should we do it in TradeService or here?
                        ExternalTradeId = item.GetLong("externalTradeId"),
                        OrderId = order.Id,
    ...
    public async Task<ExternalQueryState> CancelOrder(Order order)
    {
        ...
        var state = ExternalQueryStates.CancelOrders(order.SecurityCode, content, connId, order).RecordTimes(rtt, swTotal);
    ...
    private static Order[] ParseOrders(JsonArray json, Security? security)
    {
        var orders = new Order[json.Count];
        for (int i = 0; i < json.Count; i++)
        {
            JsonNode? node = json[i];
            var obj = node?.AsObject();
    ...
    private void ParseUserStreamingData(byte[] bytes)
    {
