...
public class Execution : IExternalExecutionManagement, ISupportFakeOrder
{
    ...
    {
        if (!ValidateExchange(security, ExternalActionType.CancelAllOrders, out var errorState))
            return errorState!;
    ...
    {
        if (!ValidateExchange(security, ExternalActionType.GetTrades, out var errorState))
            return errorState!;
    ...
    {
        if (!ValidateExchange(security, ExternalActionType.GetOrders, out var errorState))
            return errorState!;
    ...
    {
        if (security != null && !ValidateExchange(security, ExternalActionType.GetOrders, out var errorState))
            return errorState!;
    ...
    /// <returns></returns>
    /// <exception cref="NotImplementedException"></exception>
    public async Task<ExternalQueryState> UpdateOrder(Order order)
    {
        throw new NotImplementedException();
    }
    ...
    public async Task<ExternalQueryState> GetOrderSpeedLimit()
    {
        ...
        ResponseHandler.CheckHeaders(response);
        var ees = new ExternalQueryState
        {
            Content = 0, // TODO
                                                                  ...
                                                                  DateTime? end = null)
    {
        ...
        if (!orderId.IsValid() && start == null && end == null)
            return ExternalQueryStates.InvalidArgument(ExternalActionType.GetTrades, "Must provide at least an orderId, start or end time.");

        if (start == null && end != null)
            return ExternalQueryStates.InvalidArgument(ExternalActionType.GetTrades, "If end time is provided, start time must also be provided.");
        ...
        var state = new ExternalQueryState
        {
            ...
            StatusCode = (int)response.StatusCode,
            UniqueConnectionId = ResponseHandler.GetUniqueConnectionId(response),
            Description = $"Got {trades.Count} trades.",
        };
        swOuter.Stop();
        state.NetworkRoundtripTime = swInner.ElapsedMilliseconds;
        state.TotalTime = swOuter.ElapsedMilliseconds;
        ...
        List<Trade> Parse(string json, Security? security)
        {
            var trades = new List<Trade>();
            var rootObj = JsonNode.Parse(json)?.AsArray();
            if (rootObj == null)
    ...
    private static bool ValidateExchange(Security? security, ExternalActionType actionType, out ExternalQueryState? errorState)
    {
        errorState = null;
        if (security == null)
        {
