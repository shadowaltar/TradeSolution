...
{
    public delegate void OrderPlacedCallback(bool isSuccessful, ExternalQueryState<Order> state);
    public delegate void OrderModifiedCallback(bool isSuccessful, Order orderSent, Order orderReceived);
    public delegate void OrderCancelledCallback(bool isSuccessful, ExternalQueryState<Order> state);
