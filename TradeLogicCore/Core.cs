...
public class Core
{
    ...
    private async Task ReconcilePositions(List<Security> securities)
    {
        ...
        async Task<List<Position>> ProcessAndSavePositionAndRecord(Security security, List<Trade> trades)
        {
            ...
            // last position may be closed
            var r = positions[^1].CreateRecord();
            await Context.Storage.MoveToError()
            var pCnt = await Context.Storage.InsertMany(positions, true);
            LogPositionUpsert(pCnt, security.Code, positions[0].Id, positions[^1].Id);

            var rCnt = await Context.Storage.InsertOne(r, true);
            LogRecordUpsert(rCnt, security.Code, positions[^1].Id);
