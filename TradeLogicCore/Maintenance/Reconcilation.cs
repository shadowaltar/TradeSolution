...
public class Reconcilation
{
    ...
    //{
    //    var (tradeTable, tradeDb) = DatabaseNames.GetTableAndDatabaseName<Trade>(security.SecurityType);
    //    var (assetTable, posDb) = DatabaseNames.GetTableAndDatabaseName<Asset>(security.SecurityType);
    //    if (tradeTable.IsBlank() || assetTable.IsBlank()) throw Exceptions.NotImplemented($"Security type {security.SecurityType} is not supported.");
    ...
    //    var affectedPositions = await _storage.Read<Asset>(assetTable, posDb, whereClause);
    ...
    //        if (newId != 0)
    //            updateSqls.Add($"UPDATE {assetTable} SET StartTradeId = {newId} WHERE StartTradeId = {ap.StartTradeId}");
    //        newId = oldToNewIds.GetOrDefault(ap.EndTradeId, 0);
    //        if (newId != 0)
    //            updateSqls.Add($"UPDATE {assetTable} SET EndTradeId = {newId} WHERE EndTradeId = {ap.StartTradeId}");
    //    }
    ...
    //private async Task<List<Asset>?> FixInvalidPositionIdInTrades(Security security, int lookbackDays)
    //{
    //    var (tradeTable, tradeDb) = DatabaseNames.GetTableAndDatabaseName<Trade>(security.SecurityType);
    //    var (assetTable, posDb) = DatabaseNames.GetTableAndDatabaseName<Asset>(security.SecurityType);
    //    if (tradeTable.IsBlank() || assetTable.IsBlank()) throw Exceptions.NotImplemented($"Security type {security.SecurityType} is not supported.");
    ...
    //    var positions = await _storage.Read<Asset>(assetTable, posDb, $"SecurityId = {security.Id} AND AccountId = {_context.AccountId} {positionInClause}");
